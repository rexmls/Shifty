...
var crypto_data = {
    ...
    NXT: { symbol: "NXT", name: "Nxt", icon: "<img src='images/coins/nxt.png'>" },
    XRP: { symbol: "XRP", name: "Ripple", icon: "<img src='images/coins/ripple.png'>" },
    ETH: { symbol: "ETH", name: "Ether", icon: "<img src='images/coins/ether.png'>" },
    BTS: { symbol: "BTS", name: "Bitshares", icon: "<img src='images/coins/bitshares.png'>" },
    DASH: { symbol: "DASH", name: "Dash", icon: "<img src='images/coins/dash.png'>" }
};
...
function getRates(rate_pair) {
	    ...
	    $.get(ssio_protocol + "shapeshift.io/rate/" + pair, function(response) {
	        ...
	        $(".exchange-rate p").text("1 ETH = " + formatted_rate + " " + altcoin_symbol.toUpperCase());
...
function pay_button_clicked(event) {
    ...
    var bitcoin_icon = crypto_data["ETH"].icon;
	var email = $("#email").val();
    var pair = currency + "_eth";
    ...
    $.post(ssio_protocol + url, data).done(function(response) {
        ...
        if(amount) {
	        ...
	        var second_inst = "It will be converted into " + bitcoin_icon + " Ether, and sent to<br>" + "<span class='depo2-address'>" + btc_address + "</span>";
        }
